...
if [ "$usepack" = "y" ]; then
	...
	declare -a REMOTEFILES=($SWFTOOLS $ALFREPOWAR $ALFSHAREWAR $SPP $GOOGLEDOCSREPO $GOOGLEDOCSSHARE $SOLRCONFIG $SOLRWAR $SOLR4CONFIG $SOLR4WAR $CAS_DOWNLOAD)
else
	# export ALF_HOME=/opt/alfresco
	...
	declare -a REMOTEFILES=($TOMCAT_DOWNLOAD $JDBCPOSTGRESURL/$JDBCPOSTGRES $JDBCMYSQLURL/$JDBCMYSQL $LIBREOFFICE $SWFTOOLS $ALFREPOWAR $ALFSHAREWAR $SPP $GOOGLEDOCSREPO $GOOGLEDOCSSHARE $SOLRCONFIG $SOLRWAR $SOLR4CONFIG $SOLR4WAR $CAS_DOWNLOAD)
fi
